...
Daily $r_p = \sum_i w_i r_i$ with today's weights, compounded $R = \prod (1 + r_t) - 1$; annualised $(1+R)^{252/n} - 1$ (the simple $R \times 252/n$ is shown too: the gap is the compounding effect). Sharpe $= (R_p - R_f)/\sigma_p$, Treynor $= (R_p - R_f)/\beta$, Jensen's $\alpha = R_p - [R_f + \beta (R_m - R_f)]$, XIRR from dated cash flows (negative for a loss), $R_f$ = Nifty 1D Rate index. CML through the Nifty 500 TRI, with the 11 stocks, their long-only efficient frontier, the tangency portfolio and the **global minimum variance portfolio (GMVP)**, long-only and within the 5-15% limits (`output/cml.png`). `output/portfolio_weights_compared*.csv` compares our weights with the GMVPs and the tangency portfolio (volatility, return, Sharpe, effective number of stocks $1/\sum w^2$, largest weight). Until the 28-Sep snapshot these are a backtest of a portfolio chosen with hindsight.
