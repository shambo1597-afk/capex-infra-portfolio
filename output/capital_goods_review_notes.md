...
54 constituents (+0.00% over this window). It is the relevant measure once the
...
- **RRG (Relative Rotation Graph).** x = RS (pp, 63 sessions); y = RS-Momentum = that RS today
  ...
  Capital Goods average (`rs_momentum_vs_sector`, `rrg_quadrant_vs_sector`).
...
Plots: `rrg_capital_goods_vs_sector.png` and the combined `rrg_all_vs_nifty500.png`.
